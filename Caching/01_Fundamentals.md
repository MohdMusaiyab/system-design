...
The way we think about it is: *If we did this expensive operation once, and we’re likely to need that exact result again in the near future, why not store a copy somewhere faster, so we don’t have to do the whole thing over?*
...
Caching is **not** persistent storage. If we lose our prep bowl, the onions are still in the fridge. A cache is ephemeral. Data in a cache is a copy of the source of truth, never the source of truth itself. If we lose the cache, the system should recover gracefully by going back to the source.
...
A single database query might take `5ms`. If we run a high-traffic endpoint that handles `1,000` requests per second, and every request does one `5ms` DB query, we’re spending `5,000ms` (5 full seconds) of blocked time every second just waiting on I/O. Our CPU cores sit idle, waiting for bytes over the wire.
...
3. **Save Cost:**
   This is the business reality. If our cache absorbs 90% of the reads, we can provision a much smaller (cheaper) database replica. Or, we can delay having to shard the database for another 6 months. Caching turns expensive, scarce resources (database IOPS, network bandwidth) into cheap resources (application memory).
